...
const MessageInput = (props) => {
	// #-- Create state variable to keep track of what's typed in
	const [messageText, setMessageText] = useState("");

	const formSubmitted = async (event) => {
		// #23 Prevent form from refreshing page
		event.preventDefault();

		//#24A Make API call via POST request to messages
		const response = await fetch(`http://localhost:3001/message`, {
			method: "POST", //#24B Make sure it is POST
			headers: {
				//#24C Let the server know JSON is in the body
				"Content-Type": "application/json",
			},
			body: JSON.stringify({
				//#24D: Convert into json
				text: messageText, //#24E: Include what the user typed in via state
				timestamp: new Date(),
				sent: true,
			}),
		});
	...
	return (
		// #22 User clicks button or hits enter to submit the form
